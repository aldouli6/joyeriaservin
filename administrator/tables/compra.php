 ...
 * @since  1.6
 */
class ServinTablecompra extends JTable
{
	...
	{
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'ServinTablecompra', array('typeAlias' => 'com_servin.compra'));
		parent::__construct('#__servin_compras', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_servin.compra.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_servin/access.xml',
				"/access/section[@name='compra']/"
			);
			$default_actions = JAccess::getAssetRules('com_servin.compra.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_servin.compra.' . (int) $this->$k;
	}
