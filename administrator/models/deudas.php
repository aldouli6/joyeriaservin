	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(CONCAT(`#__servin_proveedores_2984816`.`empresa`, \' \', `#__servin_proveedores_2984816`.`nombre`) LIKE ' . $search . '  OR  a.fecha_compra LIKE ' . $search . '  OR  a.fecha_limite LIKE ' . $search . '  OR  a.total LIKE ' . $search . '  OR  a.abono LIKE ' . $search . '  OR  a.saldo LIKE ' . $search . '  OR  a.resumen LIKE ' . $search . '  OR  a.estatus LIKE ' . $search . ' )');
			}
		}
		...
		foreach ($items as $oneItem)
		{
			...
			if (isset($oneItem->proveedor))
			{
				...
				$oneItem->proveedor = !empty($textValue) ? implode(', ', $textValue) : $oneItem->proveedor;
					...
					$oneItem->estatus = ($oneItem->estatus == '') ? '' : JText::_('COM_SERVIN_DEUDAS_ESTATUS_OPTION_' . strtoupper($oneItem->estatus));
		}
